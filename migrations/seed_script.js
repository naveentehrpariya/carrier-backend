...
async function seed() {
  ...
  if (!customer) {
    ...
    const custDoc = await Customer.create({
      ...
      status: "active",
      created_by: company.added_by
    });
    customer = custDoc.toObject();
  }
  ...
  if (!owner) {
    ...
    const ownerDoc = await OwnerOperator.create({
      tenantId, 
      company: company._id, 
      fullName: "Dummy Owner", 
      email: "dummy@owner.com", 
      phone: "+91 9876543210",
      ownerOperatorId: "OO-1001",
      status: "active", 
      currency: "CAD", 
      created_by: company.added_by
    });
  ...
  if (!truck) {
    ...
    const truckDoc = await Truck.create({
      tenantId, 
      company: company._id, 
      unitNumber: "TRK-01", 
      plateNumber: "PL-01", 
  ...
  for (let i = 0; i < 50; i++) {
    ...
    const orderData = {
      ...
      customer: customer._id, company_name: customer.company_name,
