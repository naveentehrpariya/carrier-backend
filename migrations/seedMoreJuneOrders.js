...
console.log('=== ADD MORE SPECIFIC ORDERS IN JUNE 2026 ===');
...
async function seedMoreJuneOrders() {
  ...
  console.log(`\nCreating 50 more specific orders for JUNE 2026 starting from serial_no: ${serialBase}...`);
  const ordersCreated = [];
  ...
  for (let i = 0; i < 50; i++) {
    ...
    // Random Date in June 2026
    const randomDay = Math.floor(Math.random() * 30) + 1;
    const randomHour = Math.floor(Math.random() * 12) + 8; // 8 AM to 8 PM
    const randomMinute = Math.floor(Math.random() * 60);
    // Month is 0-indexed, so 5 = June
    const createdAt = new Date(Date.UTC(2026, 5, randomDay, randomHour, randomMinute, 0));
  ...
  console.log(`\n✓ Successfully created ${ordersCreated.length} more orders for JUNE 2026!`);
...
seedMoreJuneOrders().catch(console.error);
